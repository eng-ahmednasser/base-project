...
<div class="row">
    <div class="col-md-12">
        <form method="post" action="{{ route('admin.ticket.store') }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('post')
            <div class="card ">
                        ...
                        <div class="col-sm-7">
                            <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                                <textarea class=" form-control{{ $errors->has('description') ? ' is-invalid' : '' }}"
                                     ...
                                     name="description"  required="true">{{ old('description') }}</textarea>
                        ...
                        <div class="col-sm-7">
                            <div class="form-group{{ $errors->has('finalDate') ? ' has-danger' : '' }}">
                                <input class="text-center form-control{{ $errors->has('finalDate') ? ' is-invalid' : '' }}" name="finalDate"
                                    id="input-finalDate" type="date" placeholder="{{ __('finalDate') }}" value="{{ old('finalDate') }}"
                                    required="true" aria-required="true" />
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-7">
                            <div class="form-group{{ $errors->has('user') ? ' has-danger' : '' }}">
                                {!! Form::select('user', $users, null, ['class'=>'form-control '.($errors->has('user') ?
                                '
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-7">
                            <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                                {!! Form::select('status', $status, null, ['class'=>'form-control '.($errors->has('status') ?'is-invalid' : ''),'id'=>'input-status' ,'placeholder'=>__('Select status') ]) !!}
